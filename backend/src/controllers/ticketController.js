...
exports.showTicket = async(req,res)=>{
    const {passenger_id,trip_id,issue_date,issue_time} = req.query
    const user = req.user
    let query1 = "SELECT * FROM `passenger` WHERE passenger_id = ?"
    ...
    database.query(query1,[user.ID],(err,results)=>{
        ...
        if(results && results[0].passenger_id === user.ID ){
            ...
            let query2 = "SELECT * FROM ticket,passenger,trip,train WHERE   train.train_id = trip.train_id AND ticket.passenger_id= passenger.passenger_id AND trip.trip_id = ? and passenger.passenger_id = ? and issue_date=? and issue_time=?;"
            database.query(query2,[trip_id,passenger_id,issue_date,issue_time],(err,results)=>{
